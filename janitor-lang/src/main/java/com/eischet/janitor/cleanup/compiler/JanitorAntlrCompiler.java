...
import com.eischet.janitor.cleanup.compiler.ast.expression.binary.*;
import com.eischet.janitor.cleanup.compiler.ast.expression.literal.*;
import com.eischet.janitor.cleanup.compiler.ast.expression.unary.LogicalNot;
import com.eischet.janitor.cleanup.compiler.ast.expression.unary.Negation;
import com.eischet.janitor.cleanup.compiler.ast.function.ScriptFunction;
import com.eischet.janitor.cleanup.compiler.ast.statement.*;
import com.eischet.janitor.cleanup.compiler.ast.statement.assignment.*;
import com.eischet.janitor.cleanup.compiler.ast.statement.controlflow.*;
import com.eischet.janitor.cleanup.compiler.ast.statement.specops.PostfixDecrement;
import com.eischet.janitor.cleanup.compiler.ast.statement.specops.PostfixIncrement;
import com.eischet.janitor.cleanup.compiler.ast.statement.specops.PrefixDecrement;
import com.eischet.janitor.cleanup.compiler.ast.statement.specops.PrefixIncrement;
...
import com.eischet.janitor.cleanup.compiler.ast.expression.ternary.TernaryOperation;
import com.eischet.janitor.cleanup.tools.JStringUtilities;
import janitor.lang.JanitorBaseVisitor;
...
public class JanitorAntlrCompiler extends JanitorBaseVisitor<Ast> implements JanitorCompiler {
    ...
    public JanitorAntlrCompiler(final ScriptModule module, final JanitorCompilerSettings options, final String source) {
        this.module = module;
    ...
    public Script visitScript(final JanitorParser.ScriptContext ctx) {
        ...
        final MutableList<Statement> topLevelStatements = Lists.mutable.empty();
        ...
        return new Script(location(ctx.start, ctx.stop), topLevelStatements.toImmutable(), source);
    }
    ...
    public Block visitBlock(final JanitorParser.BlockContext ctx) {
        ...
        final MutableList<Statement> statements = Lists.mutable.empty();
        ...
        return new Block(location(ctx.start, ctx.stop), statements.toImmutable());
    }
    ...
    public StringLiteral visitStringLiteralSingle(final JanitorParser.StringLiteralSingleContext ctx) {
        ...
        return new StringLiteral(location(ctx.start, ctx.stop), JString.parseLiteral(text.substring(1, text.length() - 1)));
    }
    ...
    public StringLiteral visitStringLiteralTripleSingle(final JanitorParser.StringLiteralTripleSingleContext ctx) {
        final String text = ctx.getText();
        return new StringLiteral(location(ctx.start, ctx.stop), JString.parseLiteral(text.substring(3, text.length() - 3)));
    }
    ...
    public IfStatement visitIfStatementDef(final JanitorParser.IfStatementDefContext ctx) {
        ...
        if (ctx.ifStatementDef() != null) { // else if
            final Block elseBlock = new Block(location(ctx.start, ctx.stop), Lists.immutable.of(visitIfStatementDef(ctx.ifStatementDef())));
    ...
    public ImportClause visitImportStringSingle(final JanitorParser.ImportStringSingleContext ctx) {
        ...
        final String string = JStringUtilities.unescapeJava(text.substring(1, text.length() - 1));
    ...
    public ImportStatement visitImportStatement(final JanitorParser.ImportStatementContext ctx) {
        final MutableList<ImportClause> clauses = Lists.mutable.empty();
        ...
        return new ImportStatement(location(ctx.start, ctx.stop), clauses.toImmutable());
    ...
    public QualifiedName visitQualifiedName(final JanitorParser.QualifiedNameContext ctx) {
        final ImmutableList<String> parts = ctx.validIdentifier().stream().map(ParseTree::getText).collect(Collectors2.toImmutableList());
    ...
    public Ast visitIfThenElseExpression(final JanitorParser.IfThenElseExpressionContext ctx) {
        ...
        return new TernaryOperation.IfThenElse(location(ctx.start, ctx.stop), ifClause, thenClause, elseClause);
    }
    ...
    public AstNode visitFunctionDeclaration(final JanitorParser.FunctionDeclarationContext ctx) {
        ...
        final ImmutableList<String> finishedParams = helpExtractFormalParameters(formalParameters);
    ...
    private ImmutableList<String> helpExtractFormalParameters(final JanitorParser.FormalParametersContext formalParameters) {
    ...
    private ImmutableList<String> helpExtractFormalParametersList(final JanitorParser.FormalParameterListContext fpl) {
        final MutableList<String> parameterNames = Lists.mutable.empty();
    ...
    public Ast visitLambdaExpression(final JanitorParser.LambdaExpressionContext ctx) {
        ...
        final ImmutableList<String> lambdaParameters = helpExtractLambdaParameters(ctx.lambdaParameters());
        ...
        if (lambdaExpressionContext != null) {
            ...
            return new ScriptFunction(loc, "lambda", lambdaParameters,
                new Block(loc, Lists.immutable.of(
    ...
    private ImmutableList<String> helpExtractLambdaParameters(final JanitorParser.LambdaParametersContext ctx) {
        ...
        if (formalParams != null) {
            ...
            final ImmutableList<String> fp = helpExtractFormalParametersList(formalParams);
        ...
        if (identifiers != null) {
            ...
            final MutableList<String> names = Lists.mutable.empty();
    ...
    public Ast visitListExpression(final JanitorParser.ListExpressionContext ctx) {
        ...
        if (containedExpressions != null && !containedExpressions.isEmpty()) {
            final MutableList<Expression> elements = Lists.mutable.empty();
            ...
            return new ListLiteral(location(ctx.start, ctx.stop), elements.toImmutable());
        } else {
            return new ListLiteral(location(ctx.start, ctx.stop), Lists.immutable.empty());
        }
    ...
    public Ast visitMapExpression(final JanitorParser.MapExpressionContext ctx) {
        ...
        if (props != null && !props.isEmpty()) {
            final MutableList<MapLiteral.Preset> presets = Lists.mutable.empty();
            for (final JanitorParser.PropertyAssignmentContext prop : props) {
                ...
                if (litS != null) {
                    ...
                    lit = new StringLiteral(location(prop.start, ctx.stop), JString.parseLiteral(text.substring(1, text.length() - 1)));
                } else if (litD != null) {
                    final String text = litD.getText();
                    lit = new StringLiteral(location(prop.start, ctx.stop), JString.parseLiteral(text.substring(1, text.length() - 1)));
                } else if (litTS != null) {
                    final String text = litTS.getText();
                    lit = new StringLiteral(location(prop.start, ctx.stop), JString.parseLiteral(text.substring(3, text.length() - 3)));
                }  else if (litTD != null) {
                    final String text = litTD.getText();
                    lit = new StringLiteral(location(prop.start, ctx.stop), JString.parseLiteral(text.substring(3, text.length() - 3)));
                } else if (litIdent != null) {
                    final String text = litIdent.getText();
                    lit = new StringLiteral(location(prop.start, ctx.stop), JString.of(text));
                }
                if (lit == null) {
            ...
            return new MapLiteral(location(ctx.start, ctx.stop), presets.toImmutable());
        } else {
            return new MapLiteral(location(ctx.start, ctx.stop), Lists.immutable.empty());
        }
    }
