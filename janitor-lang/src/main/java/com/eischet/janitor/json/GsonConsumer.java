...
public class GsonConsumer implements JsonConsumer {

    private final JsonReader reader;
    private final String source;

    private GsonConsumer(final JsonReader reader, final String source) {
        this.reader = reader;
        this.source = source;
    }

    public static GsonConsumer lenient(final String source) {
        final StringReader sr = new StringReader(source);
        final JsonReader reader = new JsonReader(sr);
        reader.setLenient(true);
        return new GsonConsumer(reader, source);
    }

    public static JsonConsumer lenient(final Reader sr) {
        final JsonReader reader = new JsonReader(sr);
        reader.setLenient(true);
        return new GsonConsumer(reader, null);
    }
