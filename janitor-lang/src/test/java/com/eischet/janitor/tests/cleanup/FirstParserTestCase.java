...
import com.eischet.janitor.cleanup.api.api.errors.runtime.JanitorAssertionException;
...
import com.eischet.janitor.cleanup.repl.JanitorRepl;
import com.eischet.janitor.cleanup.runtime.*;
import com.eischet.janitor.cleanup.runtime.modules.CollectionsModule;
...
public class FirstParserTestCase {
    ...
    public void forLoops() throws JanitorCompilerException, JanitorRuntimeException {
        ...
        assertEquals("1\n2\n3\n", getOutput("for (i in list) { print(i); }", globals -> globals.bind("list", new JList(Lists.immutable.of(
            JInt.of(1),
            JInt.of(2),
            JInt.of(3)
        )))));
        // i im inneren Block ist separat vom i im äußeren Block:
        assertEquals("1\n2\n3\n4\n", getOutput("i=4; for (i in list) { print(i); } print(i);", globals -> globals.bind("list", new JList(Lists.immutable.of(
            JInt.of(1),
            JInt.of(2),
            JInt.of(3)
        )))));
    ...
    public void javaFunctionCalls() throws JanitorCompilerException, JanitorRuntimeException {
        final NativeFunction foo = new NativeFunction("foo") {
            @Override
    ...
    @Test
    public void stackTracing() throws JanitorCompilerException, JanitorRuntimeException {
        try {
            evaluate("""
                // some comment
                function fail() {
    ...
    public void modulesTest() throws JanitorCompilerException, JanitorRuntimeException {
        ...
        final JanitorScript s = rt.compile("test", """
            import foo;
            print(foo.x);
            """);
        s.run();
    ...
    public void multiImport() throws JanitorCompilerException, JanitorRuntimeException {
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        for (final String moduleName : Lists.immutable.of("foo", "bar", "baz")) {
        ...
        final JanitorScript s2 = rt.compile("test", """
            import foo, bar, baz;
            assert(foo.name == 'foo');
            assert(bar.name == 'bar');
            assert(baz.name == 'baz');
        ...
        final JanitorScript s3 = rt.compile("test", """
        ...
        final JanitorScript s4 = rt.compile("test", """
        ...
        final JanitorScript s5 = rt.compile("test", """
        ...
        rt.addModuleResolver(new BaseRuntime.ModuleResolver() {
            @Override
            public @Nullable JanitorModule resolveModuleByStringName(final JanitorScriptProcess process, final String name) throws JanitorRuntimeException {
    ...
    public void dating() throws JanitorCompilerException, JanitorRuntimeException {
        ...
        assertTrue(fiveDays instanceof JDuration);
        assertEquals(5L, ((JDuration) fiveDays).getAmount());
        assertEquals(JDurationKind.DAYS, ((JDuration) fiveDays).getUnit());
    ...
    public void regexes() throws JanitorCompilerException, JanitorRuntimeException {
        ...
        final JanitorScript phoneMatcher = rt.compile("test", "re/\\+(9[976]\\d|8[987530]\\d|6[987]\\d|5[90]\\d|42\\d|3[875]\\d|" +
    ...
    @Test
    public void calculatedPropertiesAccess() {
    }
    ...
    public void templates() throws Exception {
        final JMap huch = new JMap();
        huch.put("hello", JString.of("Huch?"));
    }

    @Test
    public void jsLikeConcatenatedIfs() throws Exception {
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();

        assertEquals(
            JBool.TRUE, rt.compile("foo", "@today+@1mi < @now").run(NO_GLOBALS)
        );
    ...
    public void longStringsCollide() throws Exception {
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        final JanitorScript script = rt.compile("test", """
            a = '''hallo''';
            c = \"\"\"ignore\"\"\";
            b = '''welt''';
            d = \"\"\"ignore\"\"\";
    ...
    public void parseLiteral() throws Exception {
        assertEquals("foo", JString.parseLiteral("foo").janitorGetHostValue());
        assertEquals("foo\\bar", JString.parseLiteral("foo\\\\bar").janitorGetHostValue());
        assertEquals("\"", JString.parseLiteral("\\\"").janitorGetHostValue());
    ...
    @Test
    public void failingAnds() throws Exception {
        final String source = """
            result = "foo";
            if (actionType.shortCode == 'EH_RUECKANTWORT') {
                print("actionType.shortCode", actionType.shortCode, "ticket.open", ticket.open, "ticket.userStatusAction", ticket.userStatusAction);
                result = (ticket.open) and (ticket.userStatusAction) and (ticket.userStatusAction.shortCode == 'EH_RUECKFRAGE');
                print("ticket.open", ticket.open, not not ticket.open);
                print("ticket.userStatusAction", ticket.userStatusAction, not not ticket.userStatusAction);
                if (ticket.userStatusAction) {
                    print("ticket.userStatusAction.shortCode", ticket.userStatusAction.shortCode, ticket.userStatusAction.shortCode=="EH_RUECKFRAGE");
                }
                print("---> result=", result);
                
                
                a = not not ticket.open;
                b = not not ticket.userStatusAction;
                c = ticket.userStatusAction.shortCode == "EH_RUECKFRAGE";
                ...
                result = ticket.open and not not ticket.userStatusAction and c;
            ...
            """;
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        final JanitorScript script = rt.compile("foo", source);

        log.info(rt.getAllOutput());
    ...
    public void communicatingScopes() throws Exception {
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        final JanitorScript firstScript = rt.compile("first", "i = 17;");
        final ResultAndScope ras = firstScript.runAndKeepGlobals(NO_GLOBALS);
        final JanitorScript secondScript = rt.compile("second", "print(i);");
    ...
    public void emptyScriptsDoNothingAtAll() throws Exception {
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        final JanitorScript firstScript = rt.compile("first", "");
        assertEquals(JNull.NULL, firstScript.run(NO_GLOBALS));
        final JanitorScript secondScript = rt.compile("second", null);
    ...
    public void stringTemplating() throws Exception {
        ...
        final JanitorScript script2 = rt.compile("test", """
            a = 1;
            print('start');
            for (i in [1, 2, 3]) {
    ...
    public void tryCatching() throws JanitorCompilerException, JanitorRuntimeException {
        ...
        final JanitorScript s3 = rt3.compile("tryCatching3", """
            try {
                print("tried");
            } catch (e) {
                print("failed");
        ...
        final JanitorScript s4 = rt4.compile("tryCatching4", """
            try {
                print("tried");
                i = 1/0;
            } catch (e) {
        ...
        final JanitorScript s2 = rt2.compile("tryCatching2", """
        ...
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        final JanitorScript script = rt.compile("tryCatching", """
    ...
    public void detectIllegalAssignment() throws Exception {
        ...
        assertThrows(JanitorNameException.class, new Executable() {
            ...
            public void execute() throws Throwable {
                ...
                final JanitorScript script = rt.compile("illegalAssignment", "1=1");
                script.run();
            }
        });
    }
    ...
    @Test
    public void sourceModules() throws Exception {
        // 2023-05-25: hier war mal column 11 drin, durch ein Umsetzen des "IP" habe ich aber daraus, besser, die 13 gemacht, was gut ist, weil "b" auch das Problem hier ist.
    }
    ...
    @Test
    public void propertyAnomaly() throws JanitorRuntimeException, JanitorCompilerException {
    ...
    public void indexAssignmentOnMap() throws JanitorCompilerException, JanitorRuntimeException {
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        rt.getCompilerSettings().setVerbose(true);
        final JanitorScript script = rt.compile("indexed_assignment", "a = {}; a['foo'] = 'bar'; print(a['foo']);");
        script.run();
        assertEquals("bar\n", rt.getAllOutput());
    }
    ...
    public void indexAssignmentOnList() throws JanitorCompilerException, JanitorRuntimeException {
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        rt.getCompilerSettings().setVerbose(true);
        final JanitorScript script = rt.compile("indexed_assignment", "a = ['baz']; a[0] = 'bar'; print(a[0]);");
        script.run();
        assertEquals("bar\n", rt.getAllOutput());
    }
    ...
    public void localKeywordsFuckedUp() throws JanitorRuntimeException, JanitorCompilerException {
        ...
        final JanitorScript script = rt.compile("from_test", """
            for (i from 1 to 10) {
                print(i);
            }
            print(from);
    ...
    public void classProperty() throws JanitorCompilerException, JanitorRuntimeException {
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        final JanitorScript script = rt.compile("class_property", """
            print((17).class);
            print(null.class);
            """);
        script.run();
    ...
    public void npeOnBoundNull() throws JanitorCompilerException, JanitorRuntimeException {
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        final JanitorScript script = rt.compile("npe_on_bound_null", """
            print("hello, world");
            """);
    ...
    @Test void dateDiff() throws JanitorRuntimeException, JanitorCompilerException {
        ...
        final OutputCatchingTestRuntime rt = new OutputCatchingTestRuntime();
        final JanitorScript script = rt.compile("diff", "return @2022-10-01-12:00:00 - @2022-10-01-11:00:00");
        final JanitorObject result = script.run();
